...
The second semester (Fall 2011) in file 1 has an empty column (column 15), so remove that.

```{r}
F11_f1<- F11_f1[,-15]
```

The first semester (Spring 2011) in file 2 is missing the "math3" column, so go back and add a column of "NA" values. 

```{r}
S11_f2$"MTH 3" <- rep(NA, nrow(S11_f2))
```

The fourth semester (Fall 2012) in file 2 has id's are too high by 1000, so subtract 1000 from the ID column.
...
We now extract the id, gender, and charachteristic from each file. 
